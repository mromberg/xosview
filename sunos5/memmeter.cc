...
void MemMeter::getmeminfo(void)
{
	...
	if (ksp_zfs) {
		if (kstat_read(kc, ksp_zfs, NULL) == -1) {
			logFatal << "Can not read vmem::zfs_file_data_buf kstat." << std::endl;
		}
		k = (kstat_named_t *)kstat_data_lookup(ksp_zfs, const_cast<char *>("mem_inuse"));
		if (k == NULL) {
			logFatal << "Can not read vmem::zfs_file_data_buf:mem_inuse kstat." << std::endl;
		}
	...
	k = (kstat_named_t *)kstat_data_lookup(ksp_sp, const_cast<char *>("pp_kernel"));
	if (k == NULL) {
		logFatal << "Can not read unix:0:system_pages:pp_kernel kstat." << std::endl;
	}
	fields_[0] = kstat_to_double(k) - fields_[1];
	k = (kstat_named_t *)kstat_data_lookup(ksp_sp, const_cast<char *>("freemem"));
	if (k == NULL) {
		logFatal << "Can not read unix:0:system_pages:freemem kstat." << std::endl;
	}
