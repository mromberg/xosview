...
void NetMeter::getnetstats( void ){
  ...
  for (unsigned int i = 0; i < _nets->count(); i++) {
    ...
    if ( (k = (kstat_named_t *)kstat_data_lookup(ksp, const_cast<char *>("rbytes64"))) == NULL ) {
        if ( (k = (kstat_named_t *)kstat_data_lookup(ksp, const_cast<char *>("rbytes"))) == NULL ) {
            if ( (k = (kstat_named_t *)kstat_data_lookup(ksp, const_cast<char *>("ipackets"))) == NULL )
          continue;
    ...
    if ( (k = (kstat_named_t *)kstat_data_lookup(ksp, const_cast<char *>("obytes64"))) == NULL ) {
        if ( (k = (kstat_named_t *)kstat_data_lookup(ksp, const_cast<char *>("obytes"))) == NULL ) {
            if ( (k = (kstat_named_t *)kstat_data_lookup(ksp, const_cast<char *>("opackets"))) == NULL )
          continue;
        strncpy(_lfr.lifr_name, ksp->ks_name, sizeof(_lfr.lifr_name));
        if ( ioctl(_socket, SIOCGLIFMTU, (caddr_t)&_lfr) < 0 )
          continue;
