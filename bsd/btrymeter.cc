...
void BtryMeter::checkResources( void ) {
	...
	leftcolor_ = parent_->g().allocColor( parent_->getResource("batteryLeftColor") );
	usedcolor_ = parent_->g().allocColor( parent_->getResource("batteryUsedColor") );
	chargecolor_ = parent_->g().allocColor( parent_->getResource("batteryChargeColor") );
	fullcolor_ = parent_->g().allocColor( parent_->getResource("batteryFullColor") );
	lowcolor_ = parent_->g().allocColor( parent_->getResource("batteryLowColor") );
	critcolor_ = parent_->g().allocColor( parent_->getResource("batteryCritColor") );
	nonecolor_ = parent_->g().allocColor( parent_->getResource("batteryNoneColor") );
