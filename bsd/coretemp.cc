...
void CoreTemp::checkResources( void ) {
	...
	actcolor_  = parent_->g().allocColor( parent_->getResource( "coretempActColor" ) );
	highcolor_ = parent_->g().allocColor( parent_->getResource( "coretempHighColor" ) );
	...
	priority_ = util::stoi( parent_->getResource( "coretempPriority" ) );
        std::string highest = parent_->getResourceOrUseDefault( "coretempHighest", "100" );
