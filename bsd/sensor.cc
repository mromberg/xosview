...
BSDSensor::BSDSensor( XOSView *parent, const std::string &name,
  ...
  const std::string &caption, int nbr )
	...
	if (high != "") {
		has_high_ = true;
		if ( sscanf(high.c_str(), "%lf", &high_) == 0 ) {  // high given as number?
			n = high;
			tmp = n.substr( 0, n.find_first_of('.') );
	...
	if (low != "") {
		has_low_ = true;
		if ( sscanf(low.c_str(), "%lf", &low_) == 0 ) {  // low given as number?
			n = low;
			tmp = n.substr( 0, n.find_first_of('.') );
...
void BSDSensor::checkResources( void ) {
	...
	actcolor_  = parent_->g().allocColor( parent_->getResource( "bsdsensorActColor" ) );
	highcolor_ = parent_->g().allocColor( parent_->getResource( "bsdsensorHighColor" ) );
	lowcolor_  = parent_->g().allocColor( parent_->getResource( "bsdsensorLowColor" ) );
	...
	char s[32];
        std::string tmp = parent_->getResourceOrUseDefault( "bsdsensorHighest", "0" );
	...
	snprintf(s, 32, "bsdsensorUsedFormat%d", nbr_);
        std::string f = parent_->getResourceOrUseDefault(s, NULL);
	setUsedFormat( f.size() ? f : parent_->getResource( "bsdsensorUsedFormat" ) );
