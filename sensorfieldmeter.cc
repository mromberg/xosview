...
void SensorFieldMeter::updateLegend( void ){
  ...
  logProblem << "FIXME: metric" << std::endl;
  total = 0; //total = scaleValue(total_, tscale, true);
  if ( (!negative_ && has_high_) || (negative_ && has_low_) ) {
    if ( ( 0.1 <= fabs(total_) && fabs(total_) < 9.95 ) ||
      ...
      if ( strlen(unit_) )
        ...
        logProblem << "FIXME: metric" << std::endl;
        limit = 0; //limit = scaleValue(limit, lscale, true);
      if ( strlen(unit_) )
...
void SensorFieldMeter::checkFields( double low, double high ){
  ...
  if (do_legend) {
    ...
    drawfields(parent_->g()); // force drawing in case only limit has changed
  }
}
