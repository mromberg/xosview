...
std::vector<std::string> WinHardware::getAdapters(void) {
    ULONG outBufLen = 0;
    ULONG flags = GAA_FLAG_SKIP_DNS_SERVER | GAA_FLAG_SKIP_UNICAST
    ...
    if(GetAdaptersAddresses(AF_UNSPEC, flags, NULL,
        (PIP_ADAPTER_ADDRESSES)addrBuf.data(), &outBufLen)
      == NO_ERROR) {
        PIP_ADAPTER_ADDRESSES pAddresses =
            (PIP_ADAPTER_ADDRESSES)addrBuf.data();
        while (pAddresses) {
            std::string desc = unwide(pAddresses->Description);
